...
gs_rx_antenna = pylink.Antenna(
    pattern=pylink.pattern_generator(48),
    rx_noise_temp_k=300,
    polarization="RHCP",
    is_rx=True,
    tracking=True,
...
sat_receiver = pylink.Receiver(
    rf_chain=sat_rf_chain,
    implementation_loss_db=2,
    name="Satellite 433 MHz LoRa Receiver",
)

gs_receiver = pylink.Receiver(rf_chain=gs_rf_chain, name="Ground 433 MHz LoRa Receiver")

gs_transmitter = pylink.Transmitter(
    tx_power_at_pa_dbw=23, name="Ground 433 MHz LoRa Transmitter"
)

sat_transmitter = pylink.Transmitter(
    tx_power_at_pa_dbw=1.5, name="Satellite 433 MHz LoRa Transmitter"
)
...
DOWNLINK = pylink.DAGModel(
    [
        ...
        tx_interconnect,
        modulation,
        pylink.LinkBudget(name="433 MHz LoRa Downlink", is_downlink=True),
    ]
)
...
UPLINK = pylink.DAGModel(
    [
        ...
        gs_tx_antenna,
        s_channel,
        pylink.LinkBudget(name="433 MHz LoRa Uplink", is_downlink=False),
    ]
)
